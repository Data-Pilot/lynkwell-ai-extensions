...
flowchart LR
  ...
  BG -->|fetch JWT + prompts| API
  API -->|generateContent| GM
  CS -->|SCRAPE_PROFILE reply| BG
...
sequenceDiagram
  ...
  SP->>BG: SCRAPE_PROFILE via tabs.sendMessage
  BG->>CS: message to tab
  CS->>Tab: read DOM scrapeProfile
  CS-->>SP: storage.session reachai_profile_v1 passive updates
  CS-->>BG: success data
  SP->>SP: renderProfile getRecommendation
```

- **Active scrape**: `chrome.tabs.sendMessage(tabId, { action: 'SCRAPE_PROFILE' })` from [sidepanel.js](extension/sidepanel.js) `scanProfile`.
...
- Side panel: `getRecommendation` → background `GET_RECOMMENDATION`.
...
- Server uses JSON output mode for `recommend` ([buildGeminiGenerationConfig](server/index.js)).
...
This is the full active application surface area for this repo (14 JS/HTML/CSS/JSON files under extension + server, plus marketing).
