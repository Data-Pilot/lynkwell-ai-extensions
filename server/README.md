...
- **Branded home:** open the server root (e.g. [http://127.0.0.1:3847/](http://127.0.0.1:3847/)) — HTML landing page with **Linkwell Connect** logo (`server/public/linkwell-connect-logo.png`), coral / navy palette aligned with `extension/sidepanel.css`. Static files under `server/public/` are served from the site root (e.g. `/linkwell-connect-logo.png`). **[`/security`](http://127.0.0.1:3847/security)** — security overview. Responses include baseline **security headers** (CSP on HTML, `X-Frame-Options`, etc.).
